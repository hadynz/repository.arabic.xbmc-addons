...
def CATEGORIES():
	...
	#addDir('افلام كرتون ','http://tv1.alarab.net/view-1_%D8%A7%D9%81%D9%84%D8%A7%D9%85-%D8%B9%D8%B1%D8%A8%D9%8A%D8%A9_1',4,'http://www.alfnnews.com/files/pic/2012/5/19/2012561916144-alarb.gif')
...
def list_films(url):
    ...
    for i in range(1,20):
        ...
        url_ch=(re.compile('<a href="(.+?)" title="مشاهدة فيلم(.+?)">').findall(link))
        for items in url_ch:
           
            if "http://tv1.alarab.net" not in items[0]:
                if "كامل اون لاين مباشرة يوتيوب كواليتي عالية على العرب بدون تحميل مشاهدة مباشرة مشاهدة فيلم" in items[1]: 
                    title=str( items[1]).split("كامل اون لاين مباشرة يوتيوب كواليتي عالية على العرب بدون تحميل مشاهدة مباشرة مشاهدة فيلم")
                elif "كامل مترجم للعربية اون لاين مباشرة كواليتي عالية على العرب بدون تحميل مشاهدة مباشرة مشاهدة فيلم" in items[1]: 
                    title=str( items[1]).split("كامل مترجم للعربية اون لاين مباشرة كواليتي عالية على العرب بدون تحميل مشاهدة مباشرة مشاهدة فيلم")
                #print title[0]
                addDir(title[0],"http://tv1.alarab.net"+str(items[0]),2,"")
...
def get_epos_video(url,name):
    ...
    url_ch="http://flv-origin.alarab.net//flv/"+str(url_ch)
